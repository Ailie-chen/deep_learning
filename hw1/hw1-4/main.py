...
def train(network, train_data, train_labels, test_data, test_labels, 
          batch_size=64, num_epochs=10, learning_rate=0.01, lr_decay="linear"):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print("Epoch %d Train Loss %.5f Train Acc %.5f Eval Loss %.5f Eval Acc %.5f LR %.5f" % \
              (epoch, train_loss, train_acc, test_loss, test_acc, learning_rate))
        
        # 学习率衰减
        if lr_decay == "linear":
            learning_rate -= (origin_lr) / num_epochs
        elif lr_decay == "cosine":
            learning_rate = origin_lr * (math.cos((epoch / num_epochs) * math.pi) + 1) / 2
...
if __name__ == "__main__":
    ...
    loss_train1, acc_train1, loss_val1, acc_val1 = train(model1, train_data, train_labels, 
                                                         ...
                                                         learning_rate=5e-1, lr_decay="linear")
    plot_learning_curve(loss_train1, acc_train1, loss_val1, acc_val1, "hw1-4/linear_lr_decay_curve.png")
    ...
    loss_train2, acc_train2, loss_val2, acc_val2 = train(model2, train_data, train_labels, 
                                                         test_data, test_labels, num_epochs=20, 
                                                         learning_rate=5e-1, lr_decay="cosine")

    plot_learning_curve(loss_train2, acc_train2, loss_val2, acc_val2, "hw1-4/cosine_lr_decay_curve.png")
