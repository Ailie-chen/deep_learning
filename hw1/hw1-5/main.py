    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in range(num_batches):
            ...
            for layer in network.layers:
                if weight_decay == "L1":
                    # L1 正则化 Loss = CE_Loss + |w|
                    layer.W -= learning_rate * layer.W_grad + lamb * learning_rate * np.sign(layer.W)
                else:
                    # L2 正则化 Loss = CE_Loss + 0.5 * \sum{(w^2)}
                    layer.W -= learning_rate * layer.W_grad + lamb * learning_rate * layer.W
...
if __name__ == "__main__":
    ...
    loss_train1, acc_train1, loss_val1, acc_val1 = train(model1, train_data, train_labels, 
                                                         ...
                                                         learning_rate=1e-1, weight_decay="L1", lamb=1e-5)
    plot_learning_curve(loss_train1, acc_train1, loss_val1, acc_val1, "hw1-5/l1_norm_curve.png")
    ...
    loss_train2, acc_train2, loss_val2, acc_val2 = train(model2, train_data, train_labels, 
                                                         test_data, test_labels, num_epochs=20, 
                                                         learning_rate=1e-1, weight_decay="L2", lamb=1e-4)

    plot_learning_curve(loss_train2, acc_train2, loss_val2, acc_val2, "hw1-5/l2_norm_curve.png")
