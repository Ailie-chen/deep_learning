...
def train(network, train_data, train_labels, test_data, test_labels, 
          batch_size=64, num_epochs=10, learning_rate=0.01, weight_decay="L2", lamb = 1e-4):
...
if __name__ == "__main__":
    ...
    # He均匀分布初始化
    model_he_uniform = NeuralNetwork(input_size=784, output_size=10, layer_sizes=[256, 256], init="he-uniform")
    loss_train1, acc_train1, loss_val1, acc_val1 = train(model_he_uniform, train_data, train_labels, 
                                                         test_data, test_labels, num_epochs=20, 
                                                         learning_rate=1e-1)
    plot_learning_curve(loss_train1, acc_train1, loss_val1, acc_val1, "hw1-3/he_uniform_init_curve.png")

    # He高斯分布初始化
    model_he_gauss = NeuralNetwork(input_size=784, output_size=10, layer_sizes=[256, 256], init="he-gauss")
    loss_train2, acc_train2, loss_val2, acc_val2 = train(model_he_gauss, train_data, train_labels, 
                                                         test_data, test_labels, num_epochs=20, 
                                                         learning_rate=1e-1)
    plot_learning_curve(loss_train2, acc_train2, loss_val2, acc_val2, "hw1-3/he_gauss_init_curve.png")
