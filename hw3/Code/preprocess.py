...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-root', type=str, default='dataset', help='data root dir')
    parser.add_argument('--split', type=str, default='train')
    args = parser.parse_args()

    annot_dir = os.path.join(args.data_root, f"labels/all_{args.split}")
    if not os.path.isdir(annot_dir):
        os.makedirs(annot_dir)

    mat_f = os.path.join(args.data_root, f"{args.split}/digitStruct.mat")
    mat = h5py.File(mat_f)
    ...
    for idx in range(data_size):
        ...
        img_path = os.path.join(args.data_root, args.split, img_name)
